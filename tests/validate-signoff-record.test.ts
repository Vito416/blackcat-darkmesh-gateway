...
function validRecord(overrides: Partial<Record<string, string>> = {}) {
  const values = {
    ...
    migrationSummary: 'kernel-migration/FINAL_MIGRATION_SUMMARY.md',
    checklist: 'kernel-migration/DECOMMISSION_CHECKLIST.md',
    decision: 'GO',
    ...
    rollbackDoc: 'kernel-migration/DECOMMISSION_CHECKLIST.md',
  ...
  return [
    ...
    `| Release evidence ledger | \`${merged.ledgerTime}\` | \`kernel-migration/release-evidence-ledger.json\` | Reviewed |`,
    `| Release drill manifest | \`${merged.manifestTime}\` | \`kernel-migration/release-drill-manifest.json\` | Reviewed |`,
    `| AO dependency gate validation | \`${merged.aoGateTime}\` | \`kernel-migration/ao-dependency-gate.json\` | Reviewed |`,
