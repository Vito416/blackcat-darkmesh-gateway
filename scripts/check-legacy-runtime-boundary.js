...
const LEGACY_SEGMENT_RE = /(?:^|\/)(?:libs\/legacy|kernel-migration\/legacy-archive\/snapshots)(?:\/|$)/
...
function usage(exitCode = 0) {
  console.log(
    [
      'Usage:',
      '  node scripts/check-legacy-runtime-boundary.js [--root <dir>] [--json] [--strict] [--help]',
      '',
      'Options:',
...
function isForbiddenSpecifier(specifier) {
  ...
  return LEGACY_SEGMENT_RE.test(normalized)
}

function findLegacySpecifierFindings(text, filePath) {
  const findings = []
  const seen = new Set()
...
async function checkLegacyRuntimeBoundary(scanRoot) {
  ...
  for (const file of files) {
    ...
    findings.push(...findLegacySpecifierFindings(text, displayPath))
  }
...
function renderHuman(report, strict) {
  const lines = []
  lines.push('# Legacy Runtime Boundary')
  lines.push('')
  ...
  if (report.findingCount === 0) {
    lines.push('No forbidden legacy runtime imports found.')
    lines.push('')
    return `${lines.join('\n')}\n`
  }
...
export async function runCli(argv = process.argv.slice(2)) {
  ...
  const report = await checkLegacyRuntimeBoundary(args.root)
...
export { checkLegacyRuntimeBoundary, parseArgs, renderHuman }
