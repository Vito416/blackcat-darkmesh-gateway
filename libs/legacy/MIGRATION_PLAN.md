...
| `blackcat-core` | Shared kernel primitives and low-level utilities | PHP | `src/runtime/core/` + `src/runtime/template/` | High | High | P0 | `partially extracted` | Core groundwork is present (`src/runtime/core/bytes.ts`, `src/runtime/core/json.ts`, `src/runtime/core/hash.ts`, `src/runtime/core/index.ts`, `src/runtime/template/actions.ts`, `src/runtime/template/validators.ts`), including the gateway-owned canonical JSON primitive, byte-limit-safe parsing helpers, and deterministic SHA-256 content hashing; the primitive-by-primitive map is now machine-readable in `kernel-migration/core-primitive-map.json` (`byte helpers`, `json parsing`, `canonical json`, `hash primitives`, `template helpers`), and decommission still requires the mapped tests (`tests/runtime-core-bytes.test.ts`, `tests/runtime-core-json.test.ts`, `tests/runtime-core-canonicalJson.test.ts`, `tests/runtime-core-hash.test.ts`, `tests/template-api.test.ts`, `tests/validate-template-backend-contract.test.ts`) plus `rg -n "libs/legacy/blackcat-core" src` returning no matches. |
...
| `blackcat-installer` | Environment bootstrap and module installation workflow | PHP + shell helpers | `ops/bootstrap/` or docs-only (no request-path runtime target) | High | Low | P2 | `pending (do-not-port candidate)` | Installer logic remains ops-only, boundary checks pass (`npm run ops:check-installer-runtime-boundary -- --strict`), `rg -n "blackcat-installer|libs/legacy/blackcat-installer" src` returns no matches, and operator docs point to gateway-owned scripts/runbooks under `ops/` + `scripts/`. |
