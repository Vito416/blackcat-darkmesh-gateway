...
| `blackcat-core` | Shared kernel primitives and low-level utilities | PHP | `src/runtime/core/` + `src/runtime/template/` | High | High | P0 | `in progress` | Core groundwork is present (`src/runtime/core/bytes.ts`, `src/runtime/template/actions.ts`, `src/runtime/template/validators.ts`), decommission requires primitive-by-primitive replacement coverage (`tests/runtime-core-bytes.test.ts`, `tests/template-api.test.ts`, `tests/validate-template-backend-contract.test.ts`) and `rg -n "libs/legacy/blackcat-core" src` returning no matches. |
...
| `blackcat-auth-js` | TypeScript/JavaScript auth SDK/client helpers | TypeScript / JavaScript | `src/clients/auth-sdk/` boundary | Medium | Medium | P1 | `in progress` | A committed gateway-owned auth client boundary exists (`src/clients/auth-sdk/` or equivalent documented boundary), boundary contract tests pass, and `rg -n "libs/legacy/blackcat-auth-js" src` returns no matches. |
| `blackcat-crypto-js` | TypeScript/JavaScript crypto SDK/client helpers | TypeScript / JavaScript | `src/clients/crypto-sdk/` boundary | Medium | Medium | P1 | `in progress` | A committed gateway-owned crypto client boundary exists (`src/clients/crypto-sdk/` or equivalent documented boundary), boundary contract tests pass, and `rg -n "libs/legacy/blackcat-crypto-js" src` returns no matches. |
| `blackcat-mailing` | Outbox, SMTP transport, queue worker | PHP | `src/runtime/mailing/` | Medium | Medium | P1 | `in progress` | Gateway mailing ownership includes payload/sanitization plus queue+transport boundaries (`src/runtime/mailing/payloadPolicy.ts`, `src/runtime/mailing/sanitizer.ts`, `src/runtime/mailing/queue.ts`, `src/runtime/mailing/transport.ts`), focused queue/transport + delivery-path tests pass, and `rg -n "libs/legacy/blackcat-mailing" src` returns no matches. |
| `blackcat-gopay` | Payment gateway integration and idempotent payment flow | PHP | `src/runtime/payments/` | Medium | High | P1 | `in progress` | Gateway payment boundary includes provider/validator helpers plus GoPay webhook verification (`src/runtime/payments/providers.ts`, `src/runtime/payments/validators.ts`, `src/runtime/payments/gopayWebhook.ts`, `/webhook/gopay` in `src/handler.ts`); decommission requires focused GoPay webhook/idempotency tests and `rg -n "libs/legacy/blackcat-gopay" src` returning no matches. |
