...
final class TrustKernel
{
    ...
    public function check(): TrustKernelStatus
    {
        ...
        if ($rpcOkNow && $snapshot !== null) {
            ...
            $policyOk = false;
            /** @var 'strict'|'less-strict'|'warn' $derivedEnforcement */
            ...
            if (hash_equals(Bytes32::normalizeHex($this->config->policyHashV1), $activePolicyHash)) {
                ...
                $policyOk = true;
                $derivedEnforcement = 'strict';
            } elseif (hash_equals(Bytes32::normalizeHex($this->config->policyHashV2LessStrict), $activePolicyHash)) {
                $policyOk = true;
                $derivedEnforcement = 'less-strict';
            ...
            } elseif (hash_equals(Bytes32::normalizeHex($this->config->policyHashV3LessStrict), $activePolicyHash)) {
                $policyOk = true;
                $derivedEnforcement = 'less-strict';
                $requiresRuntimeConfigAttestation = true;
                $runtimeConfigAttestationKeyForCheck = $this->config->runtimeConfigAttestationKey;
            ...
            } elseif (hash_equals(Bytes32::normalizeHex($this->config->policyHashV3LessStrictV2), $activePolicyHash)) {
                $policyOk = true;
                $derivedEnforcement = 'less-strict';
                $requiresRuntimeConfigAttestation = true;
                $runtimeConfigAttestationKeyForCheck = $this->config->runtimeConfigAttestationKeyV2;
            ...
            } elseif (hash_equals(Bytes32::normalizeHex($this->config->policyHashV4LessStrict), $activePolicyHash)) {
                $policyOk = true;
                $derivedEnforcement = 'less-strict';
                $requiresRuntimeConfigAttestation = true;
                $runtimeConfigAttestationKeyForCheck = $this->config->runtimeConfigAttestationKey;
                ...
                $requiresPhpFingerprintAttestation = true;
                $requiresImageDigestAttestation = true;
            } elseif (hash_equals(Bytes32::normalizeHex($this->config->policyHashV4LessStrictV2), $activePolicyHash)) {
                $policyOk = true;
                $derivedEnforcement = 'less-strict';
                $requiresRuntimeConfigAttestation = true;
                $runtimeConfigAttestationKeyForCheck = $this->config->runtimeConfigAttestationKeyV2;
                ...
                $requiresPhpFingerprintAttestation = true;
                $requiresImageDigestAttestation = true;
            } elseif (hash_equals(Bytes32::normalizeHex($this->config->policyHashV5LessStrict), $activePolicyHash)) {
                $policyOk = true;
                $derivedEnforcement = 'less-strict';
                $requiresRuntimeConfigAttestation = true;
                $runtimeConfigAttestationKeyForCheck = $this->config->runtimeConfigAttestationKey;
                ...
                $requiresPhpFingerprintAttestation = true;
                $requiresImageDigestAttestation = true;
            } elseif (hash_equals(Bytes32::normalizeHex($this->config->policyHashV5LessStrictV2), $activePolicyHash)) {
                $policyOk = true;
                $derivedEnforcement = 'less-strict';
                $requiresRuntimeConfigAttestation = true;
                $runtimeConfigAttestationKeyForCheck = $this->config->runtimeConfigAttestationKeyV2;
            ...
            if ($this->effectiveEnforcement !== 'warn' && ($endpointCount < 2 || $this->config->rpcQuorum < 2)) {
                $addError('rpc_quorum_insecure', 'Strict/less-strict requires at least 2 RPC endpoints and quorum >= 2.');
            }
            ...
            //
            // This is evaluated only for HTTP request contexts.
            //
            // In CLI it is disabled by default to avoid breaking tooling/tests, but unit tests may
            // explicitly opt-in by setting: $_SERVER['BLACKCAT_TRUSTKERNEL_TEST_FORCE_HARDENING']='1'.
            $forceCliHardening = PHP_SAPI === 'cli'
                && ($_SERVER['BLACKCAT_TRUSTKERNEL_TEST_FORCE_HARDENING'] ?? null) === '1';
            if ($requestId !== null && (PHP_SAPI !== 'cli' || $forceCliHardening)) {
                $usedDoctor = false;
                ...
                try {
                    ...
                    if ($doctorFindings !== null) {
                        ...
                        if ($this->effectiveEnforcement === 'less-strict') {
                            unset($fatalStrict['php_cgi_fix_pathinfo_enabled']);
                        }
                        ...
                        foreach ($doctorFindings as $f) {
                            ...
                            if ($code === 'php_cgi_fix_pathinfo_enabled') {
                                $cgiFixPathInfoEnabled = true;
                            }

                            $isFatal = ($severity === 'error') || isset($fatalStrict[$code]);

                            if ($this->effectiveEnforcement !== 'warn' && $isFatal) {
                ...
                if (!$usedDoctor) {
                    ...
                    if ($this->effectiveEnforcement === 'less-strict') {
                        unset($fatalStrict['cgi_fix_pathinfo_enabled']);
                    }
                    ...
                    try {
                        ...
                        if (is_array($findings)) {
                            foreach ($findings as $f) {
                                ...
                                if ($code === 'cgi_fix_pathinfo_enabled') {
                                    $cgiFixPathInfoEnabled = true;
                                }
            ...
            $requiresCgiFixPathInfoProbeAttestation = ($this->effectiveEnforcement === 'less-strict') && $cgiFixPathInfoEnabled;
            if ($requiresCgiFixPathInfoProbeAttestation) {
                $requiredAttestationKeys[] = $this->config->cgiFixPathInfoProbeAttestationKeyV1;
            }
            ...
            if ($requiresCgiFixPathInfoProbeAttestation) {
                try {
                    $key = Bytes32::normalizeHex($this->config->cgiFixPathInfoProbeAttestationKeyV1);
                    $expected = Bytes32::normalizeHex($this->config->cgiFixPathInfoProbeNoExecAttestationValueV1);

                    $att = $probeAttestations[$key] ?? null;
                    if (!is_array($att) || !isset($att['value'], $att['locked']) || !is_string($att['value']) || !is_bool($att['locked'])) {
                        $addError(
                            'cgi_fix_pathinfo_probe_missing',
                            'cgi.fix_pathinfo is enabled; less-strict requires a locked "NO EXEC" probe attestation.',
                        );
                    } else {
                        $onChain = Bytes32::normalizeHex($att['value']);
                        $zero = '0x' . str_repeat('00', 32);
                        if (hash_equals($zero, $onChain)) {
                            $addError(
                                'cgi_fix_pathinfo_probe_missing',
                                'cgi.fix_pathinfo is enabled; less-strict requires a locked "NO EXEC" probe attestation.',
                            );
                        } elseif (!hash_equals($expected, $onChain)) {
                            $addError(
                                'cgi_fix_pathinfo_probe_mismatch',
                                'cgi.fix_pathinfo probe attestation mismatch (expected NO EXEC).',
                            );
                        }

                        if (!$att['locked']) {
                            $addError(
                                'cgi_fix_pathinfo_probe_unlocked',
                                'cgi.fix_pathinfo probe attestation key is not locked.',
                            );
                        }
                    }
                } catch (\Throwable $e) {
                    $addError('cgi_fix_pathinfo_probe_failed', 'cgi.fix_pathinfo probe attestation check failed: ' . $e->getMessage());
                }
            }
                ...
                && $this->lastOkStatus?->paused === false
                    ...
                    if (is_string($lastOkPolicyHash) && trim($lastOkPolicyHash) !== '') {
                        ...
                        $v3 = [
                            ...
                            Bytes32::normalizeHex($this->config->policyHashV3LessStrict),
                            Bytes32::normalizeHex($this->config->policyHashV3Warn),
                            Bytes32::normalizeHex($this->config->policyHashV3StrictV2),
                            Bytes32::normalizeHex($this->config->policyHashV3LessStrictV2),
                        ...
                        $v4 = [
                            ...
                            Bytes32::normalizeHex($this->config->policyHashV4LessStrict),
                            Bytes32::normalizeHex($this->config->policyHashV4Warn),
                            Bytes32::normalizeHex($this->config->policyHashV4StrictV2),
                            Bytes32::normalizeHex($this->config->policyHashV4LessStrictV2),
                        ...
                        $v5 = [
                            ...
                            Bytes32::normalizeHex($this->config->policyHashV5LessStrict),
                            Bytes32::normalizeHex($this->config->policyHashV5Warn),
                            Bytes32::normalizeHex($this->config->policyHashV5StrictV2),
                            Bytes32::normalizeHex($this->config->policyHashV5LessStrictV2),
    ...
    private function hydrateLastOkFromDiskIfAvailable(): void
    {
        ...
        $dir = dirname($path);
        if (
            $this->effectiveEnforcement !== 'warn'
            && (!is_int($euid) || $euid !== 0)
        ) {
            if (!is_int($euid)) {
