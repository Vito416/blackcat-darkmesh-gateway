...
final class HttpKernel
{
    ...
    public static function run(
        ...
        try {
            ...
            KernelErrorPage::response(500, $server, $options)->send();
        }
    }
    ...
    public static function bootstrapOrReject(
        ...
        try {
            ...
            return KernelErrorPage::response(400, $server, $options);
        }
        ...
        try {
            ...
            return KernelErrorPage::response(503, $server, $options);
        }
        ...
        if ($allowedHosts !== []) {
            ...
            if (!$ok) {
                ...
                if ($status->enforcement !== 'warn') {
                    return KernelErrorPage::response(400, $server, $options);
                }
            }
        }

        if ($options->checkTrustOnRequest && $status->enforcement !== 'warn' && !$status->readAllowed) {
        ...
        $phpRuntime = null;
        if ($options->requireRuntimeHardeningInStrict && $status->enforcement !== 'warn') {
            try {
    ...
    // Error responses are rendered by KernelErrorPage::response().
}
