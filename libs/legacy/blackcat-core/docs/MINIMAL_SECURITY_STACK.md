...
If the target server does **not** have Composer, generate the runtime config + `vendor/` on your own machine (or CI) and upload via FTP.

```bash
# Requires blackcat-cli:
vendor/bin/blackcat config runtime init --template=trust-edgen --force
# Shared hosting / FTP (portable paths):
vendor/bin/blackcat config runtime template trust-edgen-portable --json > config.runtime.json
# (then upload `config.runtime.json` next to your deployed code root, outside docroot)
```
...
```bash
vendor/bin/blackcat config runtime scan
```
