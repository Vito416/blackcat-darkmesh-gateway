...
| `blackcat-core` | `f1c3dc7` | `src/runtime/core/` + `src/runtime/template/` | `in progress` | `gateway-libs-consolidation:P0` | Core groundwork exists (`src/runtime/core/bytes.ts`, `src/runtime/core/json.ts`, `tests/runtime-core-bytes.test.ts`, `tests/runtime-core-json.test.ts`) plus template helper extraction; machine-checkable primitive evidence now lives in `kernel-migration/core-primitive-map.json` (`byte helpers`, `json parsing`, `canonical json`, `template helpers`) and keeps the remaining gap visible. |
| `blackcat-crypto` | `4f59c09` | `src/runtime/crypto/safeCompare.ts` | `partially extracted` | `gateway-libs-consolidation:P0` | Signature comparison and HMAC verification helpers are used by webhook verification (`src/runtime/crypto/safeCompare.ts`, `src/runtime/crypto/hmac.ts`) and covered by `tests/runtime-crypto-safeCompare.test.ts` + `tests/runtime-crypto-hmac.test.ts`; the broader crypto boundary still needs verification/signing-key handling. |
...
| `blackcat-installer` | `a975d15` | `ops/bootstrap/` (planned) | `pending (do-not-port candidate)` | `gateway-libs-consolidation:P2` | No `ops/bootstrap/` integration exists; treat as tooling-only until explicitly approved. |

Rollup:
- Extracted from `MANIFEST.md` modules: `8/11` (`analytics`, `auth`, `auth-js`, `config`, `crypto-js`, `gopay`, `mailing`, `sessions`).
- Partially extracted from `MANIFEST.md` modules: `1/11` (`crypto`).
- In progress from `MANIFEST.md` modules: `1/11` (`core`).
