...
| `blackcat-core` | `f1c3dc7` | `src/runtime/core/` + `src/runtime/template/` | `partially extracted` | `gateway-libs-consolidation:P0` | Core helpers are now gateway-owned across byte helpers, JSON parsing, canonical JSON, hash primitives, and template helpers; `kernel-migration/core-primitive-map.json` is the machine-readable evidence, and the remaining open item is removal evidence. |
| `blackcat-crypto` | `4f59c09` | `src/runtime/crypto/safeCompare.ts` | `partially extracted` | `gateway-libs-consolidation:P0` | Signature comparison, HMAC verification, and signature-ref helpers are used by webhook verification (`src/runtime/crypto/safeCompare.ts`, `src/runtime/crypto/hmac.ts`, `src/runtime/crypto/signatureRefs.ts`) and covered by `tests/runtime-crypto-safeCompare.test.ts` + `tests/runtime-crypto-hmac.test.ts` + `tests/runtime-crypto-signatureRefs.test.ts` + `tests/webhooks.test.ts`; the broader crypto boundary still needs verification/signing-key handling and removal evidence. |
...
- Partially extracted from `MANIFEST.md` modules: `2/11` (`core`, `crypto`).
