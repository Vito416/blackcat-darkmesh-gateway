...
This file defines the minimum removal gate for each `kernel-migration/legacy-archive/snapshots/<module>/` snapshot so weekly release review can mark modules as blocked vs removable.
...
- [ ] Update `kernel-migration/legacy-archive/MANIFEST.md` in the same PR that removes a module directory.
- [ ] Archive command output for `npm run ops:validate-legacy-manifest -- --manifest kernel-migration/legacy-archive/MANIFEST.md --legacy-dir kernel-migration/legacy-archive/snapshots --strict`.
...
- Docs required: record final destination (`src/runtime/telemetry/analytics/` or explicit do-not-port decision) in `kernel-migration/legacy-archive/MIGRATION_PLAN.md` and the module map.
- Proof required: show no request-path dependency on legacy snapshot paths for `blackcat-analytics` (`libs/legacy/blackcat-analytics` or `kernel-migration/legacy-archive/snapshots/blackcat-analytics`).
- Evidence to archive:
...
- Proof required: no runtime imports from `kernel-migration/legacy-archive/snapshots/blackcat-auth`.
- Evidence to archive:
...
- Proof required: gateway request-path runtime remains independent from `kernel-migration/legacy-archive/snapshots/blackcat-auth-js`, and any boundary stays non-request-path.
- Evidence to archive:
...
- Proof required: no runtime imports from `kernel-migration/legacy-archive/snapshots/blackcat-config`.
- Evidence to archive:
  - focused test run log
  - `rg` no-import proof log
...
- Docs required: keep `kernel-migration/core-primitive-map.json` in sync with the gateway runtime, complete primitive-by-primitive mapping in `kernel-migration/legacy-archive/MIGRATION_PLAN.md`, and record final target paths in `kernel-migration/LEGACY_MODULE_MAP.md`.
- Proof required: no hidden direct dependency on legacy snapshot paths for `blackcat-core` in request path and no backslide from runtime-core boundaries to legacy helpers.
- Evidence to archive:
...
- Proof required: no runtime imports from `kernel-migration/legacy-archive/snapshots/blackcat-crypto`, and the gateway request path remains verification-only with no wallet/private-key signing.
- Evidence to archive:
  - focused test run log
...
- Docs required: keep the gateway-owned client boundary and its current map + migration-plan status synchronized until the snapshot can be removed.
- Proof required: gateway runtime stays independent from `kernel-migration/legacy-archive/snapshots/blackcat-crypto-js`, and any boundary remains non-request-path.
- Evidence to archive:
...
- Proof required: do not remove snapshot until committed gateway-owned webhook + idempotency adapter boundaries exist and `rg -n "(?:libs/legacy|kernel-migration/legacy-archive/snapshots)/blackcat-gopay" src` returns no matches.
- Evidence to archive:
...
- Proof required: prove no direct SMTP/runtime dependency on legacy snapshot paths for `blackcat-mailing` in gateway, and prove the mailing subtree does not require local secrets (`node scripts/check-mailing-secret-boundary.js --strict` with `Findings: 0`).
- Evidence to archive:
...
- Proof required: no runtime imports from `kernel-migration/legacy-archive/snapshots/blackcat-sessions`.
- Evidence to archive:
  - focused test run log
  - `rg` no-import proof log
...
- Proof required: no request-path runtime dependency on legacy snapshot paths for `blackcat-installer`.
- Evidence to archive:
  - ops validation log
  - `rg` no-import proof log
