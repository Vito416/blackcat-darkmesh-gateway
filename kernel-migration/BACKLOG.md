...
- [ ] Latest machine-validated release evidence is archived (`release-evidence-pack.md`, `release-evidence-pack.json`, `build-release-signoff-checklist` output, `check-release-readiness --json` output, `validate-ao-dependency-gate` output, drift report `.md`, drift summary `.json`, `release-drill-manifest.json`).
- [ ] Preferred operator drill path is `scripts/run-release-drill.js`; archive the matrix, drift report/summary, release pack, signoff checklist, readiness JSON, and `release-drill-manifest.json` from one run.
- [ ] Archived drill bundle includes `release-drill-manifest.json` and strict validation output.
