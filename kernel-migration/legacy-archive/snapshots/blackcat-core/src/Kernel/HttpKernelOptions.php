...
final class HttpKernelOptions
{
    ...
    /**
     * If true, render generic kernel rejections (400/503/500) as locked-down HTML pages
     * instead of plain text.
     *
     * Security requirements:
     * - pages must not reflect user-controlled input (URI, Host, headers),
     * - no scripts,
     * - strict CSP (`default-src 'none'`).
     *
     * Keep this enabled only for human-facing installs (demo / minimal bundles).
     */
    public bool $prettyErrorPages = false;

    /**
     * Optional asset base URL for pretty error pages (e.g. "/_blackcat/assets").
     *
     * If assets are missing, pages still render using gradients.
     */
    public string $prettyErrorAssetsBase = '/_blackcat/assets';

    /** Optional hero banner filename under `$prettyErrorAssetsBase` (PNG/JPG). */
    public string $prettyErrorHeroBanner = 'hero-banner.png';

    /** Optional background grid filename under `$prettyErrorAssetsBase` (PNG). */
    public string $prettyErrorGrid = 'bg-grid.png';
}
