...
This plan is the working guide for moving code from `kernel-migration/legacy-archive/snapshots/` into gateway-owned runtime modules.
...
- `pending (do-not-port candidate)`: tooling-only or integration-only scope that should stay out of request-path runtime unless explicitly approved.
...
- every gateway import from archived legacy paths has a planned replacement path
...
- no new direct imports from `libs/legacy/` or `kernel-migration/legacy-archive/snapshots/` are added outside the integration boundary
...
- `kernel-migration/legacy-archive/snapshots/` is reference-only, or fully removed where no longer required
