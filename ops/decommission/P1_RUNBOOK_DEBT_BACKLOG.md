...
- [~] Validate and document AO resolver outage path (negative-cache TTL + circuit-breaker open/close windows).
- Owner: gateway ops.
...
- [~] Complete write-handler field-type audit for `Site-Id` and other key identity fields; keep malformed writes fail-closed.
