...
| gateway | Gateway already validates worker routing, signature refs, release-drill artifacts, bounded resource profiles, and ships trace-propagation smoke checks. | Add a signed template-config snapshot and tighten versioned bridge-contract checks as strict CI gate. | P1 | `blackcat-darkmesh-gateway` / gateway ops | `cd blackcat-darkmesh-gateway && npm run ops:check-template-worker-map-coherence -- --require-token-map --require-signature-map --strict --json && npm run ops:check-template-variant-map -- --require-sites site-alpha,site-beta --strict --json` |
