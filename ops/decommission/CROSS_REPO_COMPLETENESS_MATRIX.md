...
| --- | --- | --- | --- | --- | --- |
| write | Signed command pipeline, replay/nonce defenses, deploy + rollback runbooks, health/preflight checks, Arweave hash gating, and trace propagation to AO tags are already documented and wired. | Live canary evidence and write-intent policy map governance still need to be standardized across the full path. | P0 | `blackcat-darkmesh-write` / write ops | `cd blackcat-darkmesh-write && RUN_CONTRACTS=1 RUN_CONFLICTS=1 bash scripts/verify/preflight.sh && LUA_PATH="?.lua;?/init.lua;ao/?.lua;ao/?/init.lua" lua scripts/verify/health.lua` |
...
| worker | Inbox TTL/delete-on-download, secret-backed signer/notification boundary, `/sign` + `/notify`, metrics auth, and stress tests already exist. | Live per-site secret maps, rotation-drill evidence, and production traffic proof for the current deployment envelope are still missing. | P0 | `blackcat-darkmesh-gateway/workers/secrets-worker` / site ops | `cd blackcat-darkmesh-gateway/workers/secrets-worker && npm test && npm run test:stress` |
| gateway | Template backend contract, worker-routing/signatureRef coherence, strict release-drill tooling, deployment-profile readiness checks, decommission gates, trace propagation smoke, and site-variant smoke are implemented. | Real live-endpoint drift proof remains the main confidence task before final cutover. | P0 | `blackcat-darkmesh-gateway` / gateway ops | `cd blackcat-darkmesh-gateway && npm run ops:audit-all && npm run ops:check-production-readiness -- --json && npm run ops:validate-hosting-readiness -- --profile vps_medium --env-file config/example.env --strict --json` |

## Future-Proof Matrix

| area | implemented | missing | priority | owner | verification command |
| --- | --- | --- | --- | --- | --- |
| write | Canonical write deployment already has rollback/finalization steps and now preserves `x-trace-id` through gateway/worker/write adapter into AO tags. | Freeze write-intent policy (`signatureRef -> action/role`) as a versioned contract with CI enforcement. | P1 | `blackcat-darkmesh-write` / write ops | `cd blackcat-darkmesh-write && node scripts/build-write-bundle.js && RUN_CONTRACTS=1 RUN_CONFLICTS=1 bash scripts/verify/preflight.sh` |
...
| worker | Worker secret boundaries are explicit, and the repo already has stress/load testing plus operational env guidance. | Add a versioned `/sign` contract, secret-map rotation drill evidence, and a stronger live deployment assertion for the current site set. | P1 | `blackcat-darkmesh-gateway/workers/secrets-worker` / worker owners | `cd blackcat-darkmesh-gateway/workers/secrets-worker && npm test && npm run test:stress` |
| gateway | Gateway already validates worker routing, signature refs, release-drill artifacts, bounded resource profiles, and ships trace/variant smoke plus AO read fallback chaos probe tooling. | Add a signed template-config snapshot and tighten versioned bridge-contract checks as strict CI gate. | P1 | `blackcat-darkmesh-gateway` / gateway ops | `cd blackcat-darkmesh-gateway && npm run ops:check-template-worker-map-coherence -- --require-token-map --require-signature-map --strict --json && npm run ops:check-template-variant-map -- --require-sites site-alpha,site-beta --strict --json` |
...
| worker | The worker repo already exposes smoke, stress, and deployment commands for production-like runs. | Add a dedicated rotation checklist for per-site secrets and a single-command prod smoke wrapper. | P2 | `blackcat-darkmesh-gateway/workers/secrets-worker` / site ops | `cd blackcat-darkmesh-gateway/workers/secrets-worker && npm test && npm run test:stress` |
