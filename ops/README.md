...
- Legacy import manifest validator: `npm run ops:validate-legacy-manifest -- [--manifest kernel-migration/legacy-archive/MANIFEST.md] [--legacy-dir kernel-migration/legacy-archive/snapshots] [--strict] [--json]`.
- Legacy no-import evidence checker: `npm run ops:check-legacy-no-import-evidence -- [--root src] [--manifest kernel-migration/legacy-archive/MANIFEST.md] [--modules <csv>] [--strict] [--json]` scans `src/**` for references to legacy snapshot imports (`libs/legacy/<module>` and `kernel-migration/legacy-archive/snapshots/<module>`) and emits machine-readable evidence for the legacy boundary check; CI runs it in strict/json mode.
- Legacy module-map sync checker: `npm run ops:check-legacy-module-map-sync -- [--strict] [--json]` verifies that module names remain aligned across migration docs (`MIGRATION_PLAN`, `LEGACY_MODULE_MAP`, `LEGACY_DECOMMISSION_CONDITIONS`).
- Legacy risk audit helper: `npm run ops:audit-legacy-risk -- [--dir kernel-migration/legacy-archive/snapshots] [--strict] [--json]`.
- Legacy migration matrix generator: `npm run ops:build-legacy-migration-matrix -- [--manifest kernel-migration/legacy-archive/MANIFEST.md] [--risk <audit-json>] [--out kernel-migration/legacy-libs-matrix.md] [--json]`.
